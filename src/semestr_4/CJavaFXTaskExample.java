...
public class CJavaFXTaskExample extends Application {
            ...
            Task<List<Integer>> task = new Task<List<Integer>>() {
                ...
                protected List<Integer> call() {
                    ...
                    for (int i = 2; i < 100_000_000; i++) {
                        ...
                        ); //не сет. Создаем новый список, потому что иначе метод апдейт считает, что значение не меняется.
                        // чтобы он понимал, что список изменился, и вызывал слушателя и всякое
                    }
//                    после завершения метода изменится  значение свойства value.
//                    но мы хотим сделать так, чтобы по мере вычисления добавлялись числа
