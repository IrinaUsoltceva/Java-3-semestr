...
Самая первая лекция
     ...
     поискать студенческие штуки

     Atom - редактор

     у джава скрипта есть REPL для JS- возможность в консольке печатать команды для жс и получать результат

     чтобы открыть - нажать ф12, там будут инструменты разработчика

     в жава скрипт динамическая типизация
     у переменных нет типа
     поэтому избегать того, что переприсваивать одной переменной другой тип
    ...
    можно сайт писать на nodeJS

    ECMA Script - - в браузерах

    ES6 = ES2015
    ES7 
    если ищем, проверять, что запустится у наас
    ...
    Особенности языка
     ...
     console отладка
        console.log("..") - напишет что-нибудь
    ...
    функция в java
        ...
        в консоли console.log(f(1,2)) - напишет 3

        можно вкладываь функции
        ...
        внутри f вызывается g, снаружи уже не вызвать


        функции JS - объекты первого рода, можно хранить в переменных
